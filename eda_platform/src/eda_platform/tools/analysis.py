...
def resolve_numeric_correlation_columns(
    frame: pd.DataFrame,
    *,
    max_columns: int = _PAIRWISE_TEST_MAX_COLUMNS,
) -> tuple[list[str], list[str]]:
    """Resolve the columns a `columns=None` correlation screen actually scans.

    Returns (scanned, omitted_by_cap) using the same selection rule as
    `stat_tests.screen_correlations`, so a receipt can record the resolved scope.
    """
    numeric = [str(column) for column in frame.columns if is_numeric_dtype(frame[column])]
    return numeric[:max_columns], numeric[max_columns:]
